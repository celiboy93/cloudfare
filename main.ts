// main.ts (v1.22 - FIXES: Streaming, Safe Auth, Unique KV Keys)
import {
  S3Client,
  PutObjectCommand,
  // Multipart commands no longer needed for remote upload thanks to lib-storage
  // UploadPartCommand,
  // CreateMultipartUploadCommand,
  // CompleteMultipartUploadCommand,
} from "npm:@aws-sdk/client-s3";
// --- REFACTOR (v1.22): Import lib-storage for efficient remote streaming
...
for (const varName of REQUIRED_ENV_VARS) {
  if (!Deno.env.get(varName)) {
    ...
    // In Deno Deploy, this error will be logged and the script will fail to start.
    // For local Deno, we can explicitly exit.
...
// We can now safely use non-null assertion (!) because we've checked them.
...
// --- 5. Helper Functions ---
const MAX_DATE_MS = 9999999999999; // A large number for inverted timestamps
...
Deno.serve(async (req: Request) => {
  ...
  if (BASIC_AUTH_USER && BASIC_AUTH_PASS) {
    ...
    if (authHeader) {
      ...
      if (auth) {
        ...
        // --- FIX (v1.22): Use crypto.subtle.timingSafeEqual for security ---
        ...
        const userMatch = userBytes.length === expectedUserBytes.length &&
          crypto.subtle.timingSafeEqual(userBytes, expectedUserBytes);
        const passMatch = passBytes.length === expectedPassBytes.length &&
          crypto.subtle.timingSafeEqual(passBytes, expectedPassBytes);

        if (!userMatch || !passMatch) {
          return new Response("Unauthorized", { status: 401, headers: { 'WWW-Authenticate': 'Basic realm="Restricted Area"' },});
        }
        // --- End of Timing Safe Check ---

      } else {
        // Handle case where auth string is empty after "Basic "
        return new Response("Unauthorized", { status: 401, headers: { 'WWW-Authenticate': 'Basic realm="Restricted Area"' },});
      }
    } else {
      return new Response("Unauthorized", { status: 401, headers: { 'WWW-Authenticate': 'Basic realm="Restricted Area"' },});
  ...
  if (req.method === "POST" && url.pathname === "/upload-file") {
    // R2_BUCKET_NAME and R2_PUBLIC_URL are already checked at startup
    try {
      const formData = await req.formData();
      const file = formData.get("file") as File;
      if (!file) return Response.json({ error: "No file found." }, { status: 400 });
      
      // --- FIX (v1.22): Use Streaming Upload to avoid memory limits ---
      // const fileBuffer = await file.arrayBuffer(); // REMOVED: Avoids loading file into memory
      ...
      const putCommand = new PutObjectCommand({
        ...
        Key: fileName,
        Body: file.stream(), // CHANGED: Use the ReadableStream directly
        ContentType: contentType,
        ContentLength: file.size, // ADDED: Good practice for S3/R2
      ...
      // --- FIX (v1.22): Use a unique key for KV to prevent data loss ---
      await kv.set(["uploads", Date.now(), crypto.randomUUID()], entry);
  ...
  if (req.method === "POST" && url.pathname === "/upload-remote") {
    ...
    try {
      ...
      // --- REFACTOR (v1.22): Use lib-storage for efficient streaming multipart upload ---
      // This replaces the manual CreateMultipartUpload, UploadPart, and CompleteMultipartUpload logic
      ...
      const parallelUploads3 = new Upload({
        ...
        params: {
          Bucket: R2_BUCKET_NAME,
          Key: fileName,
          Body: remoteResponse.body as any, // Cast 'any' to bridge Deno ReadableStream and Node.js stream types
          ContentType: contentType,
          ContentDisposition: `attachment; filename="${fileName}"`,
        },
        queueSize: 4, // Number of parallel parts (adjust as needed)
        partSize: 10 * 1024 * 1024, // 10MB parts
      });

      // Optional: Log progress server-side
  ...
  if (req.method === "GET" && url.pathname.startsWith("/image/")) {
    if (!R2_PUBLIC_URL) return new Response("R2_PUBLIC_URL not set.", { status: 500 }); // Redundant, but safe
  ...
  if (req.method === "GET" && url.pathname === "/history") {
    ...
    // This kv.list logic will still work perfectly with the new unique keys
    const entries = kv.list({ prefix: ["uploads"], reverse: true });
    let count = 0;
    for await (const entry of entries) {
      const item = entry.value as any; // Cast to 'any' for simplicity, or define an interface
