// main.ts (v1.14 - FINAL: Fixed Double Encoding bug in Proxy)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
// Helper to format time for History page
...
// Helper to sanitize custom file names
function sanitizeFileName(name) {
  if (!name || name.trim() === "") return null;
  
  return name
    .replace(/\.[^/.]+$/, "")   // 1. Remove user-typed extension
    .replace(/[?&#/\\]/g, "")   // 2. Remove ONLY truly unsafe URL characters
    .replace(/[\s_]+/g, "-")   // 3. Replace spaces/underscores with hyphens
    .trim() || null;          // 4. Return null if empty
}
...
Deno.serve(async (req: Request) => {
  ...
  // --- *** FIXED (v1.14 - This is the real fix) *** ---
  if (req.method === "GET" && url.pathname.startsWith("/image/")) {
    if (!R2_PUBLIC_URL) {
      return new Response("R2_PUBLIC_URL not set.", { status: 500 });
    }
    
    // 1. Get the path segment (e.g., "/image/မြန်မာလို.mp4")
    // Deno's `url.pathname` ALREADY DECODES the URL.
    // So, fileName is "မြန်မာလို.mp4" (decoded Unicode)
    const fileName = url.pathname.substring("/image/".length);
    if (!fileName) {
      return new Response("File name not specified.", { status: 400 });
    }

    // 2. Construct the R2 URL.
    // *** BUG FIX (v1.13 -> v1.14) ***
    // The `fetch` API *itself* will correctly handle encoding the
    // Unicode path. We MUST NOT call encodeURIComponent() ourselves,
    // as that was causing a double-encoding bug.
    ...
    try {
      // 3. Fetch the (now correctly formatted) URL from R2
      ...
      // 4. Stream the response back (supports seeking)
  ...
  if (req.method === "GET" && url.pathname === "/history") {
    ...
    if (count === 0) {
      html += `<div>No upload history found.</div>`;
    }
