// main.ts (v1.4 - Added Storage Usage Display)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
// --- NEW: Cache for Storage Usage ---
// This prevents calling the API on every single page load.
...
// --- NEW: Function to get R2 Storage Usage ---
async function getR2StorageUsage() {
  ...
  // 2. Check if required secrets are missing
  if (!CLOUDFLARE_API_TOKEN || !R2_ACCOUNT_ID) {
    storageCache.data = "Error: CLOUDFLARE_API_TOKEN not set.";
    storageCache.timestamp = Date.now(); // Cache the error
    return storageCache.data;
  }
  if (!R2_BUCKET_NAME) {
  ...
  // 3. Fetch new data from Cloudflare API
  try {
    const response = await fetch(
      `https://api.cloudflare.com/client/v4/accounts/${R2_ACCOUNT_ID}/r2/analytics/storage`,
      {
        headers: {
    ...
    // Find the storage usage for the specific bucket
    ...
    if (bucketData && bucketData.totalStorage !== undefined) {
      ...
      // If bucket has 0 bytes, it might not appear in analytics yet
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "GET" && url.pathname === "/") {
    ...
    // --- NEW: Fetch storage usage before sending HTML ---
