// main.ts (v1.1 - Bug Fixes for Remote Upload and Large Files)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
Deno.serve(async (req: Request) => {
      ...
      <body>
        ...
        <script>
          ...
          function setResult(message, type) {
            resultDiv.innerHTML = \`<span class="\${type === 'error' ? 'error' : 'success'}">\${message}</span>\`;
          }
        </script>
      </body>
      </html>
  ...
  if (req.method === "POST" && url.pathname === "/upload-file") {
    if (!R2_BUCKET_NAME) return Response.json({ error: "R2_BUCKET_NAME not set." }, { status: 500 });
    ...
    try {
      ...
      // FIX: Clean the file name just in case
  ...
  if (req.method === "POST" && url.pathname === "/upload-remote") {
    ...
    try {
      ...
      // *** FIX 1: Generate a clean, safe filename ***
        ...
        while (buffer.length >= partSize) {
          ...
          const uploadPart = await s3Client.send(
            new UploadPartCommand({
              // *** FIX 2: Corrected R2_BUCKET_TAME typo ***
              Bucket: R2_BUCKET_NAME, 
              Key: fileName,
              UploadId: uploadId,
  ...
  if (req.method === "GET" && url.pathname.startsWith("/image/")) {
    ...
    // *** FIX 1 (Related): We must encode the filename for the fetch request ***
    // Although our new filenames are clean, this is safer.
    const r2Url = `https://${R2_PUBLIC_URL}/${encodeURIComponent(fileName)}`;

    try {
      const r2Response = await fetch(r2Url);
      if (!r2Response.ok) {
        // This is where the user's error came from
