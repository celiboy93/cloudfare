// main.ts (v1.6 - Fixed Authentication for Global API Key)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
const CLOUDFLARE_API_TOKEN = Deno.env.get("CLOUDFLARE_API_TOKEN"); // (The "Global API Key")
const CLOUDFLARE_EMAIL = Deno.env.get("CLOUDFLARE_EMAIL"); // *** NEW (8th) SECRET *** (Your account email)
...
// --- Function to get R2 Storage Usage (Now uses Global API Key method) ---
async function getR2StorageUsage() {
  ...
  // *** BUG FIX: Check for EMAIL, TOKEN, and ACCOUNT_ID ***
  if (!CLOUDFLARE_EMAIL || !CLOUDFLARE_API_TOKEN || !CLOUDFLARE_ACCOUNT_ID) {
    storageCache.data = "Error: Email/Token/AccountID secrets not set.";
  ...
  try {
    // *** BUG FIX: Use Global API Key (X-Auth-Key) and Email (X-Auth-Email) headers ***
    ...
    if (!response.ok) {
      ...
      console.error("Cloudflare API Error Details:", errorText);
    ...
    if (!data.result) {
        throw new Error("Cloudflare API returned unexpected data. 'result' field missing.");
    }
    ...
    storageCache.data = "Error: Failed to load usage. Check Deno Logs.";
    storageCache.timestamp = Date.now();
    return storageCache.data;
  }
}
