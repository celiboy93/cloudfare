// main.ts (v1.23 - FINAL DEFINITIVE VERSION: Fixes Sorting and adds Stream Token Auth)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
// *** NEW (v1.23) ***
const STREAM_ACCESS_TOKEN = Deno.env.get("STREAM_ACCESS_TOKEN"); 
...
// --- 4. Helper Functions ---
const MAX_DATE_MS = 9999999999999; // Base for inverted timestamp
...
// --- NEW (v1.23) --- Generate links with optional token
function generateLinks(host, fileName) {
    // If the token is set, add it to the URL, else use a fixed ID (for compatibility)
    const query = STREAM_ACCESS_TOKEN 
      ? `?token=${STREAM_ACCESS_TOKEN}` 
      : `?stream_id=lugyiapk2025`;
      
    const proxyLink = `https://${host}/image/${fileName}${query}`;
    const downloadLink = `https://${host}/download/${fileName}${query}`;
    const r2Link = `https://${R2_PUBLIC_URL}/${fileName}`; // R2 original remains without proxy host
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "GET" && url.pathname.startsWith("/image/")) {
    ...
    // *** NEW: Check for Token (for APK compatibility) ***
    const token = url.searchParams.get('token');
    if (STREAM_ACCESS_TOKEN && token !== STREAM_ACCESS_TOKEN) {
        return new Response("Invalid stream access token.", { status: 401 });
    }
  ...
  if (req.method === "GET" && url.pathname === "/history") {
    ...
    // --- FINAL FIX (v1.23): Inverted Timestamp Sort ---
    // Read from KV normally, as the key is now (MAX_DATE - timestamp)
