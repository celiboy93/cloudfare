// main.ts (v1.23 - FIX: Use JSR crypto for Deno Deploy compatibility)
import {
  S3Client,
  PutObjectCommand,
...
// --- FIX (v1.23): Import timingSafeEqual from Deno Standard Library (JSR)
  ...
  if (BASIC_AUTH_USER && BASIC_AUTH_PASS) {
    ...
    if (authHeader) {
      ...
      if (auth) {
        ...
        // --- FIX (v1.23): Use timingSafeEqual directly from JSR import ---
        ...
        const userMatch = userBytes.length === expectedUserBytes.length &&
          timingSafeEqual(userBytes, expectedUserBytes); // No 'crypto.subtle'
        const passMatch = passBytes.length === expectedPassBytes.length &&
          timingSafeEqual(passBytes, expectedPassBytes); // No 'crypto.subtle'
