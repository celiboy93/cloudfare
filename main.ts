// main.ts (v1.3 - Dual Links + Copy Buttons)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
Deno.serve(async (req: Request) => {
  ...
  // --- 3A. Serve the NEW Beautiful HTML (with Progress Bars + Copy Btns) ---
    ...
    return new Response(
      ...
      <head>
        ...
        <style>
          ...
          #progress-bar-outer {
            width: 100%; background: var(--progress-bg); border-radius: 5px; overflow: hidden;
          }
          #progress-bar-inner {
          ...
          #progress-text {
            text-align: center; margin-top: 5px; font-size: 0.9em; color: var(--text-dim);
          }
      ...
      <body>
        ...
        <script>
          ...
          // --- NEW: Copy Button Click Handler (using Event Delegation) ---
          ...
          // File Upload Logic (with XHR for %)
          ...
          fileForm.addEventListener('submit', (e) => {
            ...
            xhr.addEventListener('load', () => {
              ...
              try {
                ...
                if (xhr.status === 200) {
                  setResult(data); // Pass data object
                } else {
                  setResult(data.error, 'error'); // Pass error message
                }
              } catch (err) {
          ...
          // URL Upload Logic (with Animation)
