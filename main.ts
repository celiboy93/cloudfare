// main.ts (v1.10 - FINAL: Added Custom File Name for Remote Upload)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
function sanitizeFileName(name) {
  ...
  return name
    .replace(/\.[^/.]+$/, "") // Remove potential extension (we add our own)
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "") // Remove accents
    .replace(/[^a-zA-Z0-9\s-_]/g, "") // Keep only safe chars, space, hyphen, underscore
    .replace(/[\s_]+/g, "-") // Replace spaces/underscores with hyphens
    .trim() || null; // Return null if it's empty after sanitizing
}
...
Deno.serve(async (req: Request) => {
    ...
    return new Response(
      ...
      <head>
        ...
        <style>
          ...
          form { display: flex; flex-direction: column; gap: 1rem; } /* Reduced gap */
          #fileLabel {
      ...
      <body>
        ...
        <script>
          ...
          const nameInput = document.getElementById('nameInput'); // *** NEW (v1.10) ***
          const urlSubmitBtn = document.getElementById('urlSubmitBtn');

          urlForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const remoteUrl = urlInput.value;
            const customName = nameInput.value; // *** NEW (v1.10) ***
            if (!remoteUrl) return;
            ...
            try {
              const response = await fetch('/upload-remote', {
                ...
                // *** NEW (v1.10): Send both URL and Name ***
  ...
  if (req.method === "POST" && url.pathname === "/upload-file") {
    ...
    try {
      ...
      // *** NEW (v1.10): Sanitize the user's file name from computer ***
      ...
      const entry = {
        fileName, // Save the (potentially custom) name
        proxyUrl: proxyLink,
        r2Url: r2Link,
        createdAt: new Date(),
        source: "File Upload"
  ...
  if (req.method === "POST" && url.pathname === "/upload-remote") {
    if (!R2_BUCKET_NAME || !R2_PUBLIC_URL) return Response.json({ error: "R2 Environment Variables not set." }, { status: 500 });
    try {
      // *** NEW (v1.10): Get both 'url' and 'name' from JSON body ***
      ...
      // *** NEW (v1.10): Use custom name if provided, else fallback to UUID ***
  ...
  // --- 5D. Handle the Proxy Request (GET) (v1.8 Fix) ---
