// main.ts (v1.24 - FIX: Corrected remote URL variable and Inverted History Sort)
import {
  S3Client,
  PutObjectCommand,
...
// --- 5. Helper Functions ---
// --- FIX (v1.24): Using this variable now for inverted timestamps ---
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "POST" && url.pathname === "/upload-file") {
    try {
      ...
      // --- FIX (v1.24): Use Inverted Timestamp for guaranteed newest-first sorting ---
  ...
  if (req.method === "POST" && url.pathname === "/upload-remote") {
    try {
      ...
      // --- FIX (v1.24): Removed backslash \ from \${url.host} ---
      const proxyLink = `https://${url.host}/image/${fileName}`;
      const r2Link = `https://${R2_PUBLIC_URL}/${fileName}`;
      const downloadLink = `https://${url.host}/download/${fileName}`;
  ...
  if (req.method === "GET" && url.pathname === "/history") {
    ...
    // --- FIX (v1.24): Removed { reverse: true } to use inverted timestamp sorting ---
