// main.ts (v1.15 - FINAL: Added Basic Auth Password Protection)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
// --- NEW (v1.15): Get Auth Secrets (6th and 7th secrets) ---
...
Deno.serve(async (req: Request) => {
  ...
  // --- *** NEW (v1.15) BASIC AUTH CHECK *** ---
  // This check runs BEFORE all other code.
  if (BASIC_AUTH_USER && BASIC_AUTH_PASS) {
    const authHeader = req.headers.get("Authorization");
    
    if (authHeader) {
      const auth = authHeader.split(" ")[1]; // Get the Base64 part
      if (auth) {
        // Decode the Base64 (username:password)
        const [user, pass] = atob(auth).split(":");
        
        // Check if username and password match the secrets
        if (user === BASIC_AUTH_USER && pass === BASIC_AUTH_PASS) {
          // --- User is authorized, continue to the app ---
        } else {
          // --- Invalid credentials ---
      ...
      // --- No auth header, ask the browser to show the login box ---
      return new Response("Unauthorized", {
        status: 401,
        headers: { 'WWW-Authenticate': 'Basic realm="Restricted Area"' },
      });
  ...
  // --- 5A. Serve the Uploader HTML Page ---
        ...
        <div class="container">
          ...
          <div id="tab-url" class="tab-content">
            <form id="urlUploadForm">
              ...
              <input type="text" id="nameInput" class="input-field" placeholder="Custom file name (Optional)">
  ...
  // --- 5B. Handle File Upload (from computer) ---
  if (req.method === "POST" && url.pathname === "/upload-file") {
    // (Unchanged from v1.14)
    ...
    try {
      ...
      const putCommand = new PutObjectCommand({Bucket: R2_BUCKET_NAME,Key: fileName,Body: new Uint8Array(fileBuffer),ContentType: contentType,});
      await s3Client.send(putCommand);
      const proxyLink = `https://${url.host}/image/${fileName}`;
      const r2Link = `https://${R2_PUBLIC_URL}/${fileName}`;
      const entry = {fileName,proxyUrl: proxyLink,r2Url: r2Link,createdAt: new Date(),source: "File Upload"};
  ...
  if (req.method === "POST" && url.pathname === "/upload-remote") {
    ...
    try {
      ...
      const fileName = `${sanitizedName || crypto.randomUUID()}.${extension}`;
      const createUpload = await s3Client.send(new CreateMultipartUploadCommand({Bucket: R2_BUCKET_NAME,Key: fileName,ContentType: contentType,}));
      ...
      const proxyLink = `https://${url.host}/image/${fileName}`;
      const r2Link = `https://${R2_PUBLIC_URL}/${fileName}`;
      const entry = {fileName,proxyUrl: proxyLink,r2Url: r2Link,createdAt: new Date(),source: "Remote URL"};
      await kv.set(["uploads", Date.now()], entry);
      return Response.json({ proxyUrl: proxyLink, r2Url: r2Link });
    } catch (err) {
  ...
  // --- 5D. Handle the Proxy Request (GET) ---
  ...
  // --- 5E. Handle the History Page (GET) ---
