// main.ts (v1.24 - FINAL FIX: Corrected Sorting and UI Size)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "POST" && url.pathname === "/upload-file") {
    ...
    const host = url.host; // Capture host before async operations
    try {
      ...
      const { proxyLink, r2Link, downloadLink } = generateLinks(host, fileName);
  ...
  if (req.method === "POST" && url.pathname === "/upload-remote") {
    if (!R2_BUCKET_NAME || !R2_PUBLIC_URL) return Response.json({ error: "R2 Environment Variables not set." }, { status: 500 });
    const host = url.host; // Capture host
    try {
      ...
      return Response.json({ proxyLink, r2Link, downloadLink: downloadLink });
    } catch (err) {
  ...
  // (Token Auth is performed here)
  if (req.method === "GET" && url.pathname.startsWith("/image/")) {
    // Check if R2_PUBLIC_URL is undefined (which it is for the user)
  ...
  // --- 5F. Handle the Proxy Request (GET /download/...) ---
  ...
  if (req.method === "GET" && url.pathname === "/history") {
    ...
    // *** FINAL FIX (v1.23): Sorting logic uses reverse: true ***
