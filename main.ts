// main.ts (v1.13 - FINAL: Fixed Double Encoding bug in Proxy)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
Deno.serve(async (req: Request) => {
  ...
  // --- *** FIXED (v1.13) *** ---
  // This now properly decodes the filename from the URL path.
  if (req.method === "GET" && url.pathname.startsWith("/image/")) {
    if (!R2_PUBLIC_URL) {
      return new Response("R2_PUBLIC_URL not set.", { status: 500 });
    }
    
    // 1. Get the path segment (e.g., "မြန်မာလို.mp4" or "My-Video.mp4")
    // Deno's `url.pathname` automatically DECODES URL-encoded characters.
    let fileName = url.pathname.substring("/image/".length);
    if (!fileName) {
      return new Response("File name not specified.", { status: 400 });
    }

    // 2. Construct the R2 URL. We must RE-ENCODE the filename.
    ...
    try {
      // 3. Fetch the (now correctly encoded) URL from R2
      const r2Response = await fetch(r2Url, fetchOptions);

      if (!r2Response.ok && r2Response.status !== 206) {
        // R2 returns 404 if the object truly isn't found
