// main.ts (v1.9 - FINAL: Added KV History + Fixed Seek/MIME Bugs)
import {
  S3Client,
  PutObjectCommand,
  UploadPartCommand,
...
function mimeToExt(mimeType) {
  const mapping = {
    ...
    'image/png': 'png',
    'image/gif': 'gif',
    'application/octet-stream': 'bin' // Default fallback
  };
...
Deno.serve(async (req: Request) => {
    ...
    return new Response(
      ...
      <head>
        ...
        <style>
          ...
          h2 { margin: 0; }
          #history-link {
            font-size: 0.9em; color: var(--accent); text-decoration: none;
          }
  ...
  if (req.method === "POST" && url.pathname === "/upload-file") {
    ...
    try {
      ...
      await kv.set(["uploads", Date.now()], entry); // Use timestamp as key for sorting
  ...
  // --- 5E. *** NEW (v1.9) *** Handle the History Page (GET) ---
  if (req.method === "GET" && url.pathname === "/history") {
    ...
    // Get all entries from KV, sorted newest first
